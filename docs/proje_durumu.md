...
**Aktif sprint:** Sprint 13 — GS-051 ✅ · Sprint 1–12 tamamlandı (257 / 350 backlog puanı — %73)
...
| GET | /api/v1/geofence/subscription | Geofence aboneliğim (auth) |
| PUT | /api/v1/geofence/subscription | Geofence aboneliği upsert — opt-in konum + yarıçap (auth) |
| POST | /api/v1/geofence/dispatch | Bir olay için yakındaki abonelere alarm (admin/operator) |
| GET | /api/v1/channels | Alan kanalları (lat/lon ile mesafeye göre öneri) (auth) |
| POST | /api/v1/channels | Kanal oluştur (admin/operator) |
| POST | /api/v1/channels/{slug}/join · /leave | Kanala katıl / ayrıl (auth) |
| GET/POST | /api/v1/channels/{slug}/messages | Kanal mesaj geçmişi / gönder (auth, üye, rate-limit) |
| POST | /api/v1/channels/messages/{id}/report | Mesajı bildir (auth) |
| DELETE | /api/v1/channels/messages/{id} | Mesajı kaldır — soft delete (admin/operator) |
| POST | /api/v1/channels/{slug}/members/{user_id}/mute · /unmute | Üye sustur / aç (admin/operator) |
...
| 023–025 | `volunteer_tasks` · `emergency_reports.image_url` · `chat_messages` |
| 026 | `earthquake_notification_prefs` quiet hours kolonları (GS-102) |
| 027 | `geofence_subscriptions` tablosu (GS-023) |
| 028 | `chat_channels` + `chat_channel_memberships` + `chat_message_reports` + `chat_messages.is_removed` (GS-111) |
...
## Sprint Burn-Up (S1–S11)

| Sprint | Tema | Puan | Kümülatif | Toplam % |
|--------|------|-----:|----------:|---------:|
| S1–S4 ✅ | Foundation → Hardening | 84 | 84 | %24 |
| S5 ✅ | Güvenlik & Mimari Temel | 21 | 105 | %30 |
| S6 ✅ | Deprem Bildirimleri & Kalite | 21 | 126 | %36 |
| S7 ✅ | Gönüllü, Hasar & E2E | 21 | 147 | %42 |
| S8 ✅ | Sohbet, Analitik & Batarya | 21 | 168 | %48 |
| S9 ✅ | Off-Grid Araştırma & ADR | 21 | 189 | %54 |
| S10 ✅ | Erişilebilirlik, Canlı Stok & Veri Entegrasyonu | 21 | 210 | %60 |
| S11 ✅ | Offline QR, Sessiz Saatler & Kapasite Rotası | 26 | 236 | %67 |
| S12 ✅ | Geofencing, Offline Harita & Topluluk | 21 | 257 | %73 |
...
## Sprint 11 — Offline QR, Sessiz Saatler & Kapasite Rotası ✅

**Hedef:** QR payload imzalama, deprem sessiz saatleri, seed script birleştirme, aktivite zaman çizelgesi, kapasiteye duyarlı toplanma alanı araması, düşük güç konum stratejisi, çok dilli QR etiketleri.

| ID | Konu | Pri | Puan | Durum |
|----|------|-----|-----:|-------|
| GS-094 | Doküman tazeleme (SPRINT_PLAN.md + PRODUCT_BACKLOG.md) | Should | 3 | ✅ SPRINT_PLAN.md S1–S11 kapsamına genişletildi; PRODUCT_BACKLOG.md güncel durum snapshot'ı eklendi |
| GS-044 | Çok dilli QR kart (TR/EN i18n etiketleri) | Could | 3 | ✅ `tr.json`/`en.json`'a `qr.*` namespace eklendi; `QRCardPage` + `QRScanResultPage` `useTranslation` kullanıyor |
| GS-090 | Seed script tek kaynak | Should | 3 | ✅ `scripts/seed.py` — PostgreSQL+PostGIS ve SQLite modlarını `--mode` flag veya `DATABASE_URL` ile otomatik destekler; eski `seed_db.py`/`seed_sqlite.py` deprecated |
| GS-043 | Offline QR hayati-bilgi okuma (HMAC imzalı payload) | Should | 3 | ✅ `qr.py` HMAC-SHA256 imzalama (`QR_SIGNING_KEY` veya `JWT_SECRET`); `GET /api/v1/qr/verify?d=` online doğrulama endpoint'i; QRScanResultPage imza badge'i (doğrulandı/doğrulanamadı/çevrimdışı) |
| GS-102 | Sessiz saatler + kritik override (deprem bildirimi) | Could | 3 | ✅ Migration 026 (`quiet_hours_start`, `quiet_hours_end`, `critical_override_magnitude`); `eq_matching.py` `_in_quiet_window()`; API şemasına alanlar eklendi |
| GS-083 | Aktivite zaman çizelgesi (audit log UI) | Could | 3 | ✅ `GET /api/v1/admin/audit-log` (sayfalı, resource_type+action filtresi); AdminDashboard "Aktivite" sekmesi — zaman çizelgesi görünümü, renk kodlu eylem noktaları, filtre |
| GS-031 | Kapasiteye duyarlı rota | Could | 5 | ✅ `GET /api/v1/spatial/nearest-safe-zone` — dolu alanlar de-prioritize, inactive/closed hariç; `NearestSafeZoneResult` tipi + `geoSafeAPI.fetchNearestSafeZone()`; CitizenSearch'e "En Yakın Toplanma Alanını Bul" eklendi |
| GS-122 | Düşük güç konum stratejisi | Should | 3 | ✅ `frontend/src/hooks/useLocation.ts` — `enableHighAccuracy: false`; düşük güç modunda tek seferlik okuma; 5 dk polling normal modda; localStorage consent yönetimi |

**Toplam:** 26 puan | **Alembic:** Migration 026 eklendi

---

## Sprint 12 — Geofencing, Offline Harita & Topluluk ✅

**Hedef:** Konuma duyarlı olay alarmları, sinyalsiz çalışan tahliye haritası ve mahalle ölçekli sohbet kanalları — mevcut push/SSE/chat altyapısı üzerine.

| ID | Konu | Pri | Puan | Durum |
|----|------|-----|-----:|-------|
| GS-023 | Coğrafi alan alarmları (geofenced push) | Could | 8 | ✅ Migration 027 (`geofence_subscriptions`); `core/geofence.py` saf yüklem + sevkiyat; `GET/PUT /api/v1/geofence/subscription` (opt-in konum + yarıçap), `POST /api/v1/geofence/dispatch` (admin/operator); acil bildirim oluşturulduğunda best-effort otomatik tetikleme; `GeofenceAlertCard` (ProfilePage); `test_geofence.py` (16 test) |
| GS-033 | Offline harita tile cache | Could | 8 | ✅ Service worker `geosafe-tiles` cache (cache-first + LRU 2000 karo, alt-alan-adı kanonikleştirme); `src/offline/tileCache.ts` (alan planlama + SW mesajlaşma); `OfflineMapControl` (viewport indir + ilerleme + boyut + temizle), `OfflineIndicator`; Map.tsx'e bağlı |
| GS-111 | Mahalle kanalları (sohbet odaları) + moderasyon | Could | 5 | ✅ Migration 028 (`chat_channels`, `chat_channel_memberships`, `chat_message_reports`, `chat_messages.is_removed`); `channels.py` — nearby/join/leave/messages + moderasyon (report/remove/mute) + rate-limit (10/60s); `ChannelsPage` (konuma göre öneri, canlı SSE); `test_channels.py` (16 test) |

**Toplam:** 21 puan | **Alembic:** Migration 027 + 028 eklendi

---

## Sprint 13 — Data Intelligence & DevEx (devam ediyor)

| ID | Konu | Pri | Puan | Durum |
|----|------|-----|-----:|-------|
| GS-051 | Beceri/rol gönüllü eşleştirme | Could | 5 | ✅ Migration 029 (`volunteer_applications.primary_role`); `GET /api/v1/volunteer-tasks/{id}/candidates` (onaylı gönüllülerden beceri eşleşmesi); `VolunteerMatchCandidate` şeması; `VolunteerPage`'e Birincil Rol dropdown; `TasksPage`'e "Eşleşen Gönüllüler" butonu + modal |
| GS-063 | Talep/olay yoğunluk ısı haritası | Could | 5 | ⬜ |
| GS-091 | OpenAPI'den tipli TS client codegen | Could | 3 | ⬜ |
| GS-093 | Prod Docker parite + healthcheck | Could | 3 | ⬜ |
| GS-112 | Chat presence, okundu işareti & arama | Could | 3 | ⬜ |
...
- **GS-024** SMS fallback (op→kullanıcı)
- **GS-041** Kayıp kişi panosu · **GS-054** Bağış eşleştirme — moderasyon tasarımı gerekli
- **GS-112** Presence/okundu
- **GS-063** Yoğunluk ısı haritası · **GS-032** Erişilebilir çok duraklı rota
- **GS-091** OpenAPI tipli client · **GS-093** Prod Docker parite · **GS-008** Load test · **GS-064** Redis cache
...
- ~~GS-072 Arapça RTL~~ — projeden çıkarıldı (2026-06-04)
...
- **QR imzalama (GS-043):** `QR_SIGNING_KEY` env var ile HMAC-SHA256; yoksa `JWT_SECRET` kullanılır. `GET /api/v1/qr/verify?d=<base64>` online doğrulama endpoint'i.
- **Sessiz saatler (GS-102):** UTC saat diliminde çalışır. `quiet_hours_start=23, quiet_hours_end=7` → gece 23:00–07:00 UTC sessiz. `critical_override_magnitude` set edilmişse bu büyüklükteki depremler her zaman gönderilir.
- **Seed script (GS-090):** `python scripts/seed.py` artık tek kaynak. `--mode sqlite` ile yerel test; `--mode postgres` ile prod seed.
- **useLocation hook (GS-122):** `enableHighAccuracy: false` — pil tasarrufu. Düşük güç modunda tek seferlik konum alınır; normal modda 5 dk'da bir yenilenir. Rıza durumu localStorage'da `geosafe-location-consent` anahtarıyla saklanır.
- **Geofence alarmları (GS-023):** `core/geofence.py` `haversine_km`'i (GS-100) yeniden kullanır — tek mesafe doğruluk kaynağı. Mahremiyet: yalnızca kullanıcının opt-in verdiği kaba konum saklanır; olay konumu push gövdesine konmaz. Acil bildirim (`POST /api/v1/emergency`) oluşturulduğunda yakındaki aboneler best-effort uyarılır; VAPID yoksa veya hata olursa rapor akışı **asla** bozulmaz.
- **Offline harita (GS-033):** Service worker `geosafe-tiles` cache'i versiyonsuzdur (SW yükseltmelerinde silinmez). Tile'lar cache-first sunulur; LRU ile `MAX_TILES=2000`'e kırpılır. Leaflet a/b/c alt-alan-adlarını döndürdüğü için anahtarlar `tile.openstreetmap.org`'a kanonikleştirilir (a.tile… → tile…). Alan ön-belleğe alma sayfadan SW'ye `MessageChannel` ile yapılır; ilerleme geri bildirilir. CSP `img-src` zaten `*.tile.openstreetmap.org`'a izin verir.
- **Mahalle kanalları (GS-111):** Kanal mesajları mevcut `chat_messages` tablosunda saklanır (`room = slug`), böylece SSE yayını (`broadcast_chat_message`) ve ChatPanel mantığı yeniden kullanılır. Moderasyon: rapor (`chat_message_reports`), kaldırma (soft delete `is_removed`, geçmişten gizlenir) ve susturma (`membership.muted`). Mesaj gönderme 10/60s IP rate-limit'lidir ve önce kanala katılım gerektirir. **Not:** SSE kanalı (GS-020) kimlik doğrulamasızdır ve tüm `chat_message` olaylarını global yayınlar (GS-110 ile aynı mimari); kanal mesaj gizliliği için ileride scoped SSE düşünülebilir.
- **Sprint 12 test ortamı notu:** Frontend birim test ortamında `react-i18next`/`i18next` kurulu değilse i18n kullanan bileşen testleri ve jsdom'da `EventSource` polyfill'i olmadığı için SSE kullanan testler (opsRoutes, QRCardPage) başarısız olur — bunlar ortam kaynaklıdır, Sprint 12 koduyla ilgili değildir. `test_geofence.py` ve `test_channels.py` `TEST_DATABASE_URL` (PostGIS) ile çalışır.
